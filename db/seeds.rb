...
Course.all.each do |course|
  ...
  course.notesheets.create!(:title => "Unit #{i}", :course_id => course.id, :author_id => users[i % userCount].id, :date_created => "10/17/19", :content => "#{x}")
  course.notesheets.create!(:title => "Unit #{i+1}", :content => "#{y}", :course_id => course.id, :author_id => users[(i + 1) % userCount].id, :date_created => "10/18/19")
